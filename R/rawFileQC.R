...
#Still working on theoretical transient insert
...
.lm.correction <- function(x){
  if("LMCorrectionppm" %in% names(x)){
    ...
    res <- x %>% 
      filter(MSOrder == "Ms") %>% 
      rename(LMCorrectionppm = LMmZCorrectionppm)
  }  
...
.ms2.frequency <- function(x){
  ...
  figure +
    scale_x_continuous(breaks = xbreaks)+
    labs(title = "Cycle load plot", subtitle = "Plotting the number of MS2 scans associated with each MS1 scan")+
...
.ms.data.points <- function(x){
  ...
  res <- x %>% 
    dplyr::filter(MSOrder == "Ms") %>% 
    dplyr::select(StartTime) %>% 
    dplyr::mutate(CycleTime = (lead(StartTime) - StartTime)*60) %>% 
  ...
  figure <- ggplot(res, aes(x = StartTime, y = Points, colour = PeakWidthAtBaseline))+
    ...
    labs(title ="Point Over Chromatographic Peak", 
         subtitle = "Plotting the number of Ms data points over different preselected chromatographic peak widths")+
