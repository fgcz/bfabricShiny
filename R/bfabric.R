...
bfabric <- function(input, output, session,
  ...
  observe({
    ...
    # 
    if (isFALSE(is.null(getToken()))){
      # TODO(cp): implement token version
      print("DEBUG:")
      print(getToken())
      updateTextInput(session, "login", value="tokenLogin")
      updateTextInput(session, "webservicepassword", value="tokenWebservicepassword")
