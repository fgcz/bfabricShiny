...
shinyServer(function(input, output, session) {
  ...
  output$downloadHyStarXML <- downloadHandler(
    ...
    content = function(file) {
      ...
      rvSave <-saveXML(xml$value(), file = file, encoding="utf-8")
      ...
      rvBfabric<- bfabricShiny::uploadResource(login = login(),
                                   webservicepassword = webservicepassword(),
                                   posturl = posturl(),
                                   containerid = input$container[1],
                                   applicationid = 289,
                                   status = 'AVAILABLE',
      ...
      values$wuid <- rvBfabric[[1]]$workunit[[1]]$`_id`
      rvSave 
    }
  )
  ...
  bfabricUpload <- observeEvent(input$generate, {
    progress <- shiny::Progress$new(session = session, min = 0, max = 1)
    progress$set(message = "uploading XCalibur MS configuration to bfabric")
    on.exit(progress$close())

    res <- getBfabricContent()
    #=====write XCalibur MS config to bfbairc=========
    if (input$instrumentControlSoftware == "XCalibur"){
